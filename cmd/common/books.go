...
var (
	...
	sleepDurationBetweenRequestsSecondsStr = envvar.GetEnv("CI_SLEEP_BETWEEN_REQUESTS_SECONDS", "1")
	minSuccessThresholdStr                 = envvar.GetEnv("CI_MIN_SUCCESS_THRESHOLD", "1")
	maxIterationsStr                       = envvar.GetEnv("CI_MAX_ITERATIONS_THRESHOLD", "0") // 0 for unlimited
	bookstoreServiceName                   = envvar.GetEnv("BOOKSTORE_SVC", "bookstore")
	warehouseServiceName                   = envvar.GetEnv("WAREHOUSE_SVC", "bookwarehouse")
	bookstoreNamespace                     = envvar.GetEnv(BookstoreNamespaceEnvVar, "bookstore")
	bookwarehouseNamespace                 = envvar.GetEnv(BookwarehouseNamespaceEnvVar, "bookwarehouse")
...
func GetExpectedResponseCodeFromEnvVar(envVar, defaultValue string) int {
	expectedRespCodeStr := envvar.GetEnv(envVar, defaultValue)
